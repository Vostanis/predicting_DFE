# Knicked from: <https://github.com/LMissher/STWave/blob/main/model/models.py>
...
class Sparse_Spatial_Attention(nn.Module):
    ...
    def forward(self, x, spa_eigvalue, spa_eigvec, tem_eigvalue, tem_eigvec):
        ...
        Q_reduce = Q[
            torch.arange(B)[:, None, None], torch.arange(T)[None, :, None], M_top, :
        ]
...
class TemporalAttention(nn.Module):
    ...
    def forward(self, x, te, Mask=True):
        ...
        query = torch.cat(torch.split(query, self.d, -1), 0).permute(
            0, 2, 1, 3
        )  # [k*B,T,N,d]
        key = torch.cat(torch.split(key, self.d, -1), 0).permute(
            0, 2, 3, 1
        )  # [k*B,N,d,T]
        value = torch.cat(torch.split(value, self.d, -1), 0).permute(
            0, 2, 1, 3
        )  # [k*B,N,T,d]
        ...
        if Mask:
            ...
            zero_vec = (-(2**15) + 1) * torch.ones_like(attention).to(
                x.device
            )  # [k*B,N,T,T]
        ...
        value = torch.cat(torch.split(value, value.shape[0] // self.h, 0), -1).permute(
            0, 2, 1, 3
        )  # [B,T,N,F]
        value = self.ofc(value)
        value += x
...
class Dual_Enconder(nn.Module):
    ...
    def forward(self, xl, xh, te):
        ...
        spa_statesl = self.spatial_att_l(
            xl,
            self.spa_eigvalue,
            self.spa_eigvec.to(xl.device),
            self.tem_eigvalue,
            self.tem_eigvec.to(xl.device),
        )  # [B,T,N,F]
        spa_statesh = self.spatial_att_h(
            xh,
            self.spa_eigvalue,
            self.spa_eigvec.to(xl.device),
            self.tem_eigvalue,
...
class Adaptive_Fusion(nn.Module):
    ...
    def forward(self, xl, xh, te, Mask=True):
        ...
        keyh = torch.cat(torch.split(keyh, self.d, -1), 0).permute(
            0, 2, 3, 1
        )  # [k*B,N,d,T]
        valueh = torch.cat(torch.split(valueh, self.d, -1), 0).permute(
            0, 2, 1, 3
        )  # [k*B,N,T,d]
        ...
        if Mask:
            ...
            mask = mask.repeat(self.h * batch_size, num_vertexs, 1, 1)  # [k*B,N,T,T]
            mask = mask.to(torch.bool)
            zero_vec = (-(2**15) + 1) * torch.ones_like(attentionh).to(
                xl.device
            )  # [k*B,N,T,T]
...
class STWave(nn.Module):
    def __init__(
        ...
        self.dual_enc = nn.ModuleList(
            [
                Dual_Enconder(heads, dims, samples, localadj, spawave, temwave)
