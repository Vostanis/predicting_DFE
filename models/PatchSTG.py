# Knicked from: <https://github.com/LMissher/PatchSTG/blob/main/models/model.py>
...
class PatchSTG(nn.Module):
    ...
    def forward(self, x, te):
        ...
        orginal = torch.zeros(
            rex.shape[0], rex.shape[1], self.node_num, rex.shape[-1]
        ).to(x.device)
        orginal[:, :, self.ori_parts_idx, :] = rex[
            :, :, self.reo_parts_idx, :
        ]  # back to the original indices

        # projection decoder -> section 4.4 in paper
        pred_y = self.regression_conv(
            orginal.transpose(2, 3).reshape(orginal.shape[0], -1, orginal.shape[-2], 1)
        )
    ...
    def embedding(self, x, te):
        ...
        x1 = torch.cat(
            [x, (te[..., 0:1] / self.tod), (te[..., 1:2] / self.dow)], -1
        ).float()
        ...
        input_data = torch.cat(
            [input_data, self.time_in_day_emb[(t_i_d_data).type(torch.LongTensor)]], -1
        )

        # cat day of week embedding
        d_i_w_data = te[:, -input_data.shape[1] :, :, 1]
        input_data = torch.cat(
            [input_data, self.day_in_week_emb[(d_i_w_data).type(torch.LongTensor)]], -1
        )
